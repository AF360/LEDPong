/*
  ...
  Wesentliche LEDPong-Erweiterungen:
    ...
    - fünf Betriebsarten: Normal, Pro, Insane, Demo und Scanner
    ...
    - wiederholte Audiowiedergabe im Scanner-Modus
...
const byte KITT_FRAME_DELAY_MS = 12; // empirisch abgestimmte Scanner-Geschwindigkeit
const byte VOLUME_MIN = 1;
const byte VOLUME_MAX = 20;
const unsigned long START_TRACK_TIMEOUT_MS = 30000UL;
const int KITT_FIRST_LED = 33;             // 34th LED from the left (zero-based index 33)
const int KITT_LAST_LED = NUM_LEDS - 34;   // 34th LED from the right (index 110 with 144 LEDs)
...
// Die Werte werden weiterhin in den ursprünglichen 60-LED-Referenzeinheiten
// angegeben und später vom Sketch auf den 144-LED-Strip skaliert.
...
const PongModeConfig pongModeConfig[4] = {
  // start, step, limit, boost0, boost1, boost2, end zone at 60 LEDs/m
  {50, 1, 7, 20, 10, 5, 7},   // Normal
  {40, 2, 5, 15, 10, 5, 6},   // Pro
  {30, 3, 3, 10,  7, 3, 5},   // Insane
  {40, 1, 5, 20, 10, 5, 6}    // Demo, based on Pro
};

SoftwareSerial mySerial(rxPin,txPin);   // RX, TX

byte nfiles = 25;                       // highest mp3 file number currently used  
...
void runPongDemo();
void runKittMode();
boolean specialModeExitRequested(GameMode expectedMode);
int chooseDemoHitPosition(int direction);
void waitForPlayerButtonsReleased();
...
//---------------
void applyModeSettings(GameMode mode) {
  byte configIndex = MODE_NORMAL;

  if (mode == MODE_PRO) {
    configIndex = MODE_PRO;
  }
  else if (mode == MODE_INSANE) {
    configIndex = MODE_INSANE;
  }
  else if (mode == MODE_DEMO) {
    configIndex = MODE_DEMO;
  }

  const PongModeConfig &config = pongModeConfig[configIndex];
...
//---------------
void announceMode(GameMode mode) {
  if (mode >= MODE_NORMAL && mode <= MODE_KITT) {
    dfpExecute(0x0F, 0x01, 20 + (byte)mode);
  }
}
...
void startSelectedMode() {
  ...
  if (selectedMode == MODE_DEMO) {
    runPongDemo();
  }
  else if (selectedMode == MODE_KITT) {
    runKittMode();
  }
  else {
    activeGame = true;
...
boolean specialModeExitRequested(GameMode expectedMode) {
  ...
  if (rawMode != MODE_INVALID && rawMode != expectedMode) {
    return true;
  }

  return digitalRead(playerBtnPin[0]) == LOW || digitalRead(playerBtnPin[1]) == LOW;
}

//---------------
void waitForPlayerButtonsReleased() {
  while (digitalRead(playerBtnPin[0]) == LOW || digitalRead(playerBtnPin[1]) == LOW) {
    delay(5);
  }
}
...
//---------------
int chooseDemoHitPosition(int direction) {
  // Roughly one in eight approaches is deliberately missed so that the
  // score display and winner sequence are also demonstrated.
  if (random(8) == 0) {
    return -1;
  }

  int distanceFromEnd = random(endZoneSize);
  return direction > 0 ? NUM_LEDS - 1 - distanceFromEnd : distanceFromEnd;
}
...
void runPongDemo() {
  ...
  while (!exitRequested) {
    ...
    ballSpeed = gameSpeed;
    ballDir = playerStart == 0 ? 1 : -1;
    ballPos = playerStart == 0 ? 0 : NUM_LEDS - 1;
    previousMoveMicros = micros();

    GeneratePlayField(scoreDimBright);
    leds[ballPos] = CHSV(0, 0, brightness);
    FastLED.show();
    dfpExecute(0x0F, 0x01, 4);            // launch-ball sound

    int demoHitPosition = chooseDemoHitPosition(ballDir);

    while (true) {
      if (specialModeExitRequested(MODE_DEMO)) {
        exitRequested = true;
        break;
      }
      ...
      if ((unsigned long)(currentMicros - previousMoveMicros) >= moveIntervalMicros) {
        ...
        if (ballPos < 0 || ballPos >= NUM_LEDS) {
          dfpExecute(0x0F, 0x01, 3);        // deliberate or random miss
          break;
        }
        ...
        if (ballPos == demoHitPosition) {
          dfpExecute(0x0F, 0x01, 2);        // pong sound
          ChangeDirection();
          previousMoveMicros = micros();
          demoHitPosition = chooseDemoHitPosition(ballDir);
        }
      }
    }
    ...
    if (playerScore[0] >= winRounds || playerScore[1] >= winRounds) {
      boolean player0Won = playerScore[0] > playerScore[1];
      dfpExecute(0x0F, 0x01, 5);          // winner sound
      FastLED.clear();
      winRainbow(player0Won);
      memset(playerScore, 0, sizeof(playerScore));
      playerStart = random(2);
    }
  }

  dfpExecute(0x16, 0x00, 0x00);           // stop audio when demo is left
  FastLED.clear();
  FastLED.show();
  waitForPlayerButtonsReleased();
}

//---------------
void runKittMode() {
  int scannerPosition = KITT_FIRST_LED;
  int scannerDirection = 1;

  FastLED.clear();
  FastLED.show();

  // The DFPlayer plays independently after the serial command, so MP3 25
  // and the LED animation run in parallel. Command 0x19 enables repeat-one.
  dfpExecute(0x0F, 0x01, 25);       // play folder 01, file 025.mp3
  dfpExecute(0x19, 0x00, 0x00);     // repeat the current track continuously

  while (!specialModeExitRequested(MODE_KITT)) {
    fadeToBlackBy(leds, NUM_LEDS, 70);
    leds[scannerPosition] = CHSV(0, 255, brightness);
    FastLED.show();

    scannerPosition += scannerDirection;
    if (scannerPosition >= KITT_LAST_LED) {
      scannerPosition = KITT_LAST_LED;
      scannerDirection = -1;
    }
    else if (scannerPosition <= KITT_FIRST_LED) {
      scannerPosition = KITT_FIRST_LED;
      scannerDirection = 1;
    }

    delay(KITT_FRAME_DELAY_MS);
  }

  // Stop first, then disable repeat-one. Some DFPlayer clones may drop
  // commands sent too closely together, so leave time between both commands
  // and clear any status bytes from the SoftwareSerial receive buffer.
  dfpExecute(0x16, 0x00, 0x00);     // stop MP3 25 when KITT mode is left
  delay(200);
  clearDfPlayerInput();

  dfpExecute(0x19, 0x00, 0x01);     // disable repeat-one for all later sounds
  delay(200);
  clearDfPlayerInput();
  FastLED.clear();
  FastLED.show();
